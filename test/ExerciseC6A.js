...
contract("ExerciseC6A", async (accounts) => {
  ...
  it("contract owner can change operational", async () => {
    // ARRANGE
    let caller = accounts[0];

    // ACT
    const operational = await config.exerciseC6A.setOperational.call(false, {
      from: caller,
    });
  ...
  it("contract functions do not execute when operational is false", async () => {
    // ARRANGE
    let caller = accounts[0];
    let newUser = config.testAddresses[3];

    // ACT
    await config.exerciseC6A.setOperational.call(false, { from: caller });
    await config.exerciseC6A.registerUser.call(newUser, false, {
      from: caller,
    });
    let result = await config.exerciseC6A.isUserRegistered.call(newUser);
